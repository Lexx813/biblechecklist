  ...
  function submitComment(e: React.FormEvent) {
    ...
    addComment.mutate(
      { postId: post.id, content: commentText.trim() },
      {
        ...
        onError: () => toast.error(t("groups.failedToAddComment")),
      }
    );
  }
    ...
    <div className={`grp-post${post.is_announcement ? " grp-post--announcement" : ""}`}>
      ...
      <div className="grp-post-header">
        ...
        <div className="grp-post-meta">
          <span className="grp-post-author">{post.author?.display_name || t("groups.unknown")}</span>
          <span className="grp-post-time">{timeAgo(post.created_at)}</span>
        </div>
        {(isOwn || isAdmin) && (
          <button
            className="grp-post-delete"
            onClick={() => deletePost.mutate(post.id, { onError: () => toast.error(t("groups.failedToDeletePost")) })}
            aria-label={t("groups.deletePost")}
          >
      ...
      <div className="grp-post-actions">
        <button
          ...
          onClick={() => toggleLike.mutate(post.id, { onError: () => toast.error(t("groups.failedUpdateLike")) })}
          aria-label={post.liked_by_me ? t("groups.unlike") : t("groups.like")}
        >
        ...
        <button className="grp-post-comment-btn" onClick={() => setExpanded(v => !v)}>
          ...
          {post.comment_count > 0 ? post.comment_count : t("groups.comment")}
        </button>
      </div>
      ...
      {expanded && (
        <div className="grp-comments">
          {commentsLoading ? (
            <p className="grp-comments-loading">{t("groups.commentsLoading")}</p>
          ) : (
            comments.map(c => (
              <div key={c.id} className="grp-comment">
                ...
                <div className="grp-comment-body">
                  <div className="grp-comment-header">
                    <span className="grp-comment-author">{c.author?.display_name || t("groups.unknown")}</span>
                    ...
                    {(c.author_id === userId || isAdmin) && (
                      <button
                        ...
                        onClick={() => deleteComment.mutate(
                          ...
                          { onError: () => toast.error(t("groups.failedToDeleteComment")) }
                        )}
                        aria-label={t("groups.deleteComment")}
                      >
          ...
          <form className="grp-comment-form" onSubmit={submitComment}>
            <input
              ...
              placeholder={t("groups.writeAComment")}
              value={commentText}
              onChange={e => setCommentText(e.target.value)}
              maxLength={1000}
              aria-label={t("groups.writeACommentAria")}
            />
            <button type="submit" className="grp-comment-send" disabled={!commentText.trim() || addComment.isPending} aria-label={t("groups.send")}>
