...
export function useTimedLeaderboard(level: number | undefined) {
  return useQuery({
...
export function useUserBestTimedScore(userId: string | undefined, level: number | undefined) {
  return useQuery({
...
export function useSaveTimedScore(userId: string | undefined) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ level, score }: { level: number; score: number }) =>
      quizTimedApi.saveTimedScore(userId!, level, score),
    onSuccess: (_, { level }) => {
