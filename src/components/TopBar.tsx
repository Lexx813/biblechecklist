            ...
            {avatarOpen && (
              <div className="topbar-avatar-menu" role="menu">
                ...
                {LANGUAGES.map(l => (
                  <button
                    key={l.code}
                    className={`topbar-avatar-item${l.code === currentLang ? " topbar-avatar-item--active" : ""}`}
                    role="menuitem"
                    onClick={() => { i18n.changeLanguage(l.code); setAvatarOpen(false); }}
                  >
                    <span aria-hidden="true">{FLAGS[l.code]}</span>
                    {l.label}
                    {l.code === currentLang && <span style={{ marginLeft: "auto", fontSize: 11, color: "#a78bfa" }}>✓</span>}
                  </button>
                ))}
                <div className="topbar-avatar-divider" />
                {setDarkMode && (
