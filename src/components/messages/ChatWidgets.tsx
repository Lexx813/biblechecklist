    ...
    <div className="fc-modal-overlay fc-img-warn-overlay" onClick={onCancel}>
      <div className="fc-modal fc-img-warn-modal" role="dialog" aria-modal="true" aria-label={t("messages.imageUploadWarning")} onClick={e => e.stopPropagation()}>
        ...
        <p className="fc-img-warn-body">
          ...
          <strong> {t("messages.imageWarningProhibition")}</strong> {t("messages.imageWarningSuspension")}
        </p>
        {previewUrl && (
          <img src={previewUrl} alt={t("common.previewAlt")} className="fc-img-warn-preview" onLoad={() => URL.revokeObjectURL(previewUrl)} />
        )}
        <p className="fc-img-warn-agree">
          {t("messages.imageWarningAgree")}
        </p>
        <div className="fc-img-warn-actions">
          <button type="button" className="fc-modal-btn fc-img-warn-cancel" onClick={onCancel}>{t("common.cancel")}</button>
          <button type="button" className="fc-modal-btn fc-modal-btn--primary" onClick={onConfirm}>{t("messages.sendImage")}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fc-overlay-panel">
      <div className="fc-overlay-header">
        <button className="fc-back-btn" onClick={onClose} aria-label={t("messages.back")}>
      ...
      <div className="fc-overlay-body">
        {isLoading ? <p className="fc-empty">{t("messages.loading")}</p> : starred.length === 0 ? (
          <p className="fc-empty">{t("messages.noStarredYet")}</p>
        ) : (
          starred.map(msg => (
            <div key={msg.id} className="fc-starred-item">
              ...
              <div className="fc-starred-meta">
                ...
                <button className="fc-action-btn" onClick={() => toggleStar.mutate(msg.id)} aria-label={t("messages.removeStar")}>
  ...
  return (
    <div className="fc-overlay-panel">
      <div className="fc-overlay-header">
        ...
        <input className="fc-search-input" placeholder={t("messages.searchMessages")} value={query} onChange={handleChange} autoFocus aria-label={t("messages.searchMessagesAria")} />
      </div>
      <div className="fc-overlay-body">
        {isFetching ? <p className="fc-empty">{t("messages.searching")}</p> : debouncedQuery.length < 2 ? (
          <p className="fc-empty">{t("messages.typeToSearch")}</p>
        ) : results.length === 0 ? (
          <p className="fc-empty">{t("messages.noResultsFor", { query: debouncedQuery })}</p>
        ) : (
          results.map(msg => (
