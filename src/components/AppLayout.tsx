...
// Bottom tab bar items shown on mobile (≤1100px)
const NAV_MOBILE_TABS = [
  ...
  { key: "main",     label: "Tracker", icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> },
  { key: "forum",    label: "Forum",   icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> },
...
export default function AppLayout({ navigate, user, currentPage, children, rightPanel }: Props) {
  ...
  return (
    <div className={`al-wrap${rightPanel ? " al-wrap--with-rp" : ""}`}>
      ...
      <nav className="al-mobile-tabs" aria-label="Mobile navigation">
        {NAV_MOBILE_TABS.map(item => (
          <button
            key={item.key}
            className={`al-mobile-tab${activeKey === item.key ? " al-mobile-tab--active" : ""}`}
            onClick={() => navigate(item.key)}
            aria-current={activeKey === item.key ? "page" : undefined}
          >
            <span className="al-mobile-tab-icon">
              {item.key === "messages" && unreadMessages > 0 && (
                <span className="al-mobile-tab-badge">{unreadMessages > 99 ? "99+" : unreadMessages}</span>
              )}
              {item.icon}
            </span>
            <span className="al-mobile-tab-label">{item.label}</span>
          </button>
        ))}
      </nav>
    </div>
