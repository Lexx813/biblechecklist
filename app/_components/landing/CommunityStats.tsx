...
export default function CommunityStats({ serverUserCount }: { serverUserCount: number }) {
  ...
  return (
    <p className="m-0 flex items-center gap-[7px] text-[13px] text-[var(--lp-muted)]" style={{ minWidth: "22ch" }}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
      </svg>
      {stats.users > 0
        ? `${stats.users.toLocaleString()} ${stats.users === 1 ? "publisher" : "publishers"} worldwide`
